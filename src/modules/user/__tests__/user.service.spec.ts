...
describe('UserService', () => {
  ...
  describe('profile', () => {
    ...
    it('should throw UnauthorizedException if user does not exist', async () => {
      ...
      await expect(userService.profile('1')).rejects.toThrow(UnauthorizedException);
    });
  });
  ...
  describe('updateAvatar', () => {
    ...
    it('should throw UnauthorizedException if user does not exist', async () => {
      ...
      await expect(userService.updateAvatar('1', 'new_avatar_url')).rejects.toThrow(UnauthorizedException);
    });
  });
  ...
  describe('updatePassword', () => {
    ...
    it('should throw BadRequestException if current password does not match', async () => {
      ...
      await expect(userService.updatePassword('1', 'wrong_password', 'new_password')).rejects.toThrow(BadRequestException);
    });

    it('should throw UnauthorizedException if user does not exist', async () => {
      userRepository.findUserById.mockResolvedValue(null);

      await expect(userService.updatePassword('1', 'current_password', 'new_password')).rejects.toThrow(UnauthorizedException);
    });
  });
